class AccountTransactionServiceObject
  ...
  def self.withdraw_from_account(account,user,amount)
    ...
    credit_transaction = Transaction.new(:source => user,:target => account,:amount => amount,:transaction_type => Transaction.transaction_types[:credit], balance_after: user.balance + amount, balance_before: user.balance)
  ...
  def self.deposit_to_account(account,user,amount)
    debit_transaction = Transaction.new(:source => user,:target => account,:amount => amount,:transaction_type => Transaction.transaction_types[:debit], balance_after: user.balance - amount, balance_before: user.balance)
